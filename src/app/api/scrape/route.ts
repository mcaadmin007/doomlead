...
import { searchGoogleMaps, type OutscraperResult } from '@/lib/outscraper'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ── หัก credit ─────────────────────────────────────
    const actualCredits = mappedResults.length * (include_email ? 2 : 1)
    await admin.rpc('add_credits', {
      p_user_id: user.id,
      p_amount: -actualCredits,
      p_description: `ค้นหา "${query} ${location}" (${mappedResults.length} รายชื่อ)`,
    })
