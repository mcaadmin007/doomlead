...
const TABLE_COLS = [
  ...
  { key: 'email',           label: 'อีเมล',          pack: 'cold_email' as EnrichmentPack },
  { key: 'full_name',       label: 'ชื่อผู้ติดต่อ',  pack: 'cold_email' as EnrichmentPack },
  { key: 'contact_phone',   label: 'เบอร์ผู้ติดต่อ', pack: 'cold_calling' as EnrichmentPack },
  ...
  { key: 'company_linkedin',label: 'LinkedIn',        pack: 'cold_calling' as EnrichmentPack },
]
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <form onSubmit={handleSearch} className="bg-white border border-zinc-200 rounded-xl p-5 mb-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1.5">ประเภทธุรกิจ / คำค้นหา</label>
            ...
            {COUNT_OPTIONS.map(opt => (
              <button key={opt} type="button" onClick={() => setCount(opt)}
                className={`px-3 py-2 text-sm rounded-md border font-medium transition-colors ${
                  count === opt ? 'bg-black text-white border-black' : 'bg-white text-zinc-700 border-zinc-200 hover:border-zinc-400'}`}>
                {opt}
              </button>
            ))}
          </div>
            ...
            {(Object.entries(ENRICHMENT_PACKS) as [EnrichmentPack, typeof ENRICHMENT_PACKS[EnrichmentPack]][]).map(([key, info]) => (
              <button key={key} type="button" onClick={() => setPack(key)}
                ...
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${pack === key ? 'bg-white/20 text-white' : 'bg-zinc-100 text-zinc-600'}`}>
                ...
                <ul className="space-y-0.5">
                  ...
                  {info.includes.length > 3 && (
                    <li className={`text-xs ${pack === key ? 'text-white/60' : 'text-zinc-400'}`}>+ {info.includes.length - 3} อื่นๆ</li>
                  )}
                </ul>
              </button>
            ))}
        ...
        <div className="flex items-center justify-between pt-1 border-t border-zinc-100">
          <p className="text-sm text-zinc-500">
            ...
            {' '}· {count} รายชื่อ × {packInfo.credits_per_result} เครดิต
          </p>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-medium text-zinc-700">
              พบ <span className="font-semibold">{results.length} รายชื่อ</span> · &ldquo;{query}&rdquo; ใน {location}
            </p>
            {results.length > 0 && (
