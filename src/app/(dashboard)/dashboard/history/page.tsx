          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b border-zinc-200">
                <tr>
                  {['วันที่', 'ประเภทธุรกิจ', 'พื้นที่', 'รายชื่อ', 'อีเมล', 'เครดิต', 'สถานะ', ''].map(h => (
                  ...
                  return (
                    <tr key={job.id} className="hover:bg-zinc-50/60 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {job.include_email ? <span className="text-green-600 text-xs">✓ รวม</span> : <span className="text-zinc-300 text-xs">—</span>}
                      </td>
                      <td className="px-4 py-3 text-zinc-600">{job.credits_used}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full border ${st.color}`}>{st.label}</span>
                      </td>
                      <td className="px-4 py-3">
                        {job.status === 'done' && (
                          <a href={`/api/export?job_id=${job.id}`}
